# Generated by jeweler
# DO NOT EDIT THIS FILE DIRECTLY
# Instead, edit Jeweler::Tasks in Rakefile, and run 'rake gemspec'
# -*- encoding: utf-8 -*-

Gem::Specification.new do |s|
  s.name = %q{tapi}
  s.version = "0.2.1"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.authors = ["Martin Tepper"]
  s.date = %q{2011-03-11}
  s.description = %q{Reference client to the Travel IQ API at http://apiv3.travel-iq.com/}
  s.email = %q{developer@traveliq.net}
  s.extra_rdoc_files = [
    "LICENSE.txt",
    "README.rdoc"
  ]
  s.files = [
    ".document",
    "Gemfile",
    "Gemfile.lock",
    "LICENSE.txt",
    "README.rdoc",
    "Rakefile",
    "VERSION",
    "lib/tapi.rb",
    "lib/tapi/v3/client.rb",
    "lib/tapi/v3/configurable.rb",
    "lib/tapi/v3/errors.rb",
    "lib/tapi/v3/flights/search.rb",
    "lib/tapi/v3/generic_search.rb",
    "lib/tapi/v3/hotels/search.rb",
    "lib/tapi/v3/utils.rb",
    "lib/tapi/v3/validations.rb",
    "spec/client_spec.rb",
    "spec/configurable_spec.rb",
    "spec/flights/search_spec.rb",
    "spec/hotels/search_spec.rb",
    "spec/spec_helper.rb"
  ]
  s.homepage = %q{http://github.com/traveliq/tapi}
  s.licenses = ["MIT"]
  s.require_paths = ["lib"]
  s.rubygems_version = %q{1.6.2}
  s.summary = %q{Reference client to the Travel IQ API at http://apiv3.travel-iq.com/}
  s.test_files = [
    "spec/client_spec.rb",
    "spec/configurable_spec.rb",
    "spec/flights/search_spec.rb",
    "spec/hotels/search_spec.rb",
    "spec/spec_helper.rb"
  ]

  if s.respond_to? :specification_version then
    s.specification_version = 3

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<activesupport>, ["~> 2.3.5"])
      s.add_runtime_dependency(%q<curb>, ["~> 0.7.10"])
      s.add_runtime_dependency(%q<json>, ["~> 1.5.1"])
      s.add_development_dependency(%q<rspec>, ["~> 2.3.0"])
      s.add_development_dependency(%q<bundler>, ["~> 1.0.0"])
      s.add_development_dependency(%q<jeweler>, ["~> 1.5.2"])
      s.add_development_dependency(%q<rcov>, [">= 0"])
    else
      s.add_dependency(%q<activesupport>, ["~> 2.3.5"])
      s.add_dependency(%q<curb>, ["~> 0.7.10"])
      s.add_dependency(%q<json>, ["~> 1.5.1"])
      s.add_dependency(%q<rspec>, ["~> 2.3.0"])
      s.add_dependency(%q<bundler>, ["~> 1.0.0"])
      s.add_dependency(%q<jeweler>, ["~> 1.5.2"])
      s.add_dependency(%q<rcov>, [">= 0"])
    end
  else
    s.add_dependency(%q<activesupport>, ["~> 2.3.5"])
    s.add_dependency(%q<curb>, ["~> 0.7.10"])
    s.add_dependency(%q<json>, ["~> 1.5.1"])
    s.add_dependency(%q<rspec>, ["~> 2.3.0"])
    s.add_dependency(%q<bundler>, ["~> 1.0.0"])
    s.add_dependency(%q<jeweler>, ["~> 1.5.2"])
    s.add_dependency(%q<rcov>, [">= 0"])
  end
end

